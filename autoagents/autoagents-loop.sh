...
GH_REPO="${AGENT_GH_REPO:-AMVARA-CONSULTING/km0-opencloud}"
...
prepare_001_preflight_context() {
  ...
  {
    echo ""
    echo "=== Docker log incident heuristics (opencloud, dex, km0-web) ==="
  } >>"$ctx"
  ...
  if docker info >/dev/null 2>&1; then
    local c raw hits
    for c in opencloud-opencloud-1 opencloud-dex km0-web; do
...
Environment:
  AGENT_GH_REPO              GitHub repo (default: AMVARA-CONSULTING/km0-opencloud)
