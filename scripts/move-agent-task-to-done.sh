...
if command -v python3 >/dev/null 2>&1 && [[ -f "${REPO_ROOT}/autoagents/gh_issue_sync.py" ]]; then
  echo "Syncing GitHub issue (comment, labels, close)..."
  python3 "${REPO_ROOT}/autoagents/gh_issue_sync.py" close "$src" || \
