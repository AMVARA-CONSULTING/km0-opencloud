...
**OpenCloud:** https://cloud.km0.amvara.de · **Web:** https://km0.amvara.de  
...
| Nginx (OpenCloud) | 1.26.3 | `/etc/nginx/sites-available/opencloud` | `cloud.km0.amvara.de` → :9200 |
...
| TLS (cloud) | Let's Encrypt | `/etc/letsencrypt/live/cloud.km0.amvara.de/` | OpenCloud |
...
| `OC_DOMAIN` | `cloud.km0.amvara.de` | Hostname público de OpenCloud (no usar `km0.amvara.de`, que es la web). |
...
| `cloud.km0.amvara.de` | `/etc/letsencrypt/live/cloud.km0.amvara.de/` |
...
| Component | Role |
|-----------|------|
| nginx `cloud.km0.amvara.de` | TLS, `/dex/` → Dex, `/` → OpenCloud |
...
| OpenCloud | `OC_OIDC_ISSUER=https://cloud.km0.amvara.de/dex`, `WEB_OIDC_CLIENT_ID=opencloud-web` |

**Login entry points:**

| Method | URL |
|--------|-----|
| Picker (Google/Apple + local) | https://cloud.km0.amvara.de/login.html |
| Google / Apple (via Dex) | https://cloud.km0.amvara.de/?oidc=1 |
| Local username + password | https://cloud.km0.amvara.de/signin/v1/identifier/_/authorize?client_id=web&redirect_uri=… |
...
Config: `/opt/opencloud/dex/`. Provider picker: https://cloud.km0.amvara.de/login.html
...
3. **Stale browser session** — old tabs still call built-in `/signin/` (502). Clear site data for `cloud.km0.amvara.de` or use a private window.
